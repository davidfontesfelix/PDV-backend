...
describe('PUT /products/edit/:code', () => {
  it('should return the error: "Produto não encontrado"', async () => {
    ...
    const responser = await request(app).put('/products/edit/0123456789012').send(newData)

    expect(responser.status).toBe(400)
    expect(responser.body).toEqual({ error: 'Produto não encontrado'})
  })
})
...
describe('DELETE /products/delete/:code', () => {
  it('should return the error: "Id não foi encontrado"', async () => {
    ...
    expect(response.status).toBe(404)
    expect(response.body).toEqual({error: 'Id não foi encontrado'})
  })
})